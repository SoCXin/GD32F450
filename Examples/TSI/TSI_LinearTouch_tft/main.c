/**
  ******************************************************************************
  * @file    TSI/TSI_LinearTouch_tft/main.c 
  * @author  MCU SD
  * @version V2.0.0
  * @date    15-Jan-2016
...
/* Bar value array */
int bar_group[4] = {80,160,240,320};
...
int main(void)
{
    ...
    /* Reference cycle value acquisition and processing */
    {
        ...
        threshold2 = Sample_Refervalue_Average[2] - Sample_Refervalue_Average[1]+30;
    }
    ...
    while (1)
    {
        /* Acquisition TSI_GROUP6_PIN2 */
        {
            ...
            /* Light LED1 and LED4 */
            if((Sample_Refervalue_Average[0]-SampleCycle[TSI_PIN2]) > 0x20)
            {
        ...
        /* Acquisition TSI_GROUP6_PIN3 */
        {
            ...
            /* Light LED2 */
            if((Sample_Refervalue_Average[1]-SampleCycle[TSI_PIN3]) > 0x30)
            {
        ...
        /* Acquisition TSI_GROUP6_PIN4 */
        {
            ...
            /* Light LED3 */
            if((Sample_Refervalue_Average[2]-SampleCycle[TSI_PIN4]) > 0x50)
            {
        ...
        /* Judge specific location of TSI_GROUP6_PIN2 */
        if(touch_flag[0] == 1)
        {
        ...
        /* Clear TFT_LCD */
        if((touch_flag[0] == 0)&&(touch_flag[1] == 0)&&(touch_flag[2] == 0))
        {
            bar_value=0;
        }
    }    
...
void show_Bar(int length)
{
    ...
    for(i = 0; i < 320; i++)    
    {
        for(j = 0; j < 240; j++)  
        {
            ...
            if((i < length) && (j < (i * 3 / 8 + Y_offset)) && (j > (Y_offset )))
            {
                Lcd_WriteData(( YELLOW >> 8) & 0xFF);
                Lcd_WriteData(YELLOW  & 0xFF);
            }
            else
            {
                Lcd_WriteData((BLUE >> 8) & 0xFF);
                Lcd_WriteData(BLUE & 0xFF);
            }
        }
    }
    LCD_CS_SET;
...
void Acquisition_TSI_PIN(uint32_t TSI_groupx_pin)
{
    /* Configure the TSI Pin Channel Mode configure */
    ...
    /* Wait the specified TSI flag state: MCE and CTCF */
