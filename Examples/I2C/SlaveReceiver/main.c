...
int main(void)
{
    ...
    while(!I2C_StateDetect(BOARD_I2C, I2C_PROGRAMMINGMODE_SLAVE_RECEIVER_ADDSEND));

    for(i=0;i<16;i++)
    {
        /* Wait until the reception data register is not empty */
        while(!I2C_StateDetect(BOARD_I2C, I2C_PROGRAMMINGMODE_SLAVE_BYTE_RECEIVED));
    ...
    while(!I2C_StateDetect(BOARD_I2C, I2C_PROGRAMMINGMODE_SLAVE_STOP_DETECTED));
